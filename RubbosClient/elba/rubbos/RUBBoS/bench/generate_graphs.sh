...
if [ -d $1 ]; then
  ...
  #################################
  ## Graphs for Servlets Controller ##
  #################################
if [ -f $1servlets_server.gz ]; then
  # Plot CPU idle time
  echo "Generating Servlets Server's CPU idle time graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_cpu_idle.$2'"' >> $tmpFile;
  echo 'set title "Processor idle time"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Processor idle time in %"' >> $tmpFile;
  echo "set yrange [0:100]" >> $tmpFile;
  echo -n plot '"'$1servlets_server.cpu.idle.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot CPU busy time 
  echo "Generating Servlets Server's CPU busy time graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_cpu_busy.$2'"' >> $tmpFile;
  echo 'set title "Processor usage"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Processor usage in %"' >> $tmpFile;
  echo "set yrange [0:100]" >> $tmpFile;
  echo -n plot '"'$1servlets_server.cpu.busy.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot CPU user/system time
  echo "Generating Servlets Server's CPU user/system time graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_cpu_user_kernel.$2'"' >> $tmpFile;
  echo 'set title "User/Kernel processor usage"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Processor usage in %"' >> $tmpFile;
  echo "set yrange [0:100]" >> $tmpFile;
  echo -n plot '"'$1servlets_server.cpu.user.dat'"' title '"'Servlets Server user'"' with lines, '"'$1servlets_server.cpu.system.dat'"' title '"'Servlets Server kernel'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Processes/second
  echo "Generating Servlets Server's Processes/second graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_procs.$2'"' >> $tmpFile;
  echo 'set title "Processes created"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of processes created per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.proc.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Context switches/second
  echo "Generating Servlets Server's Context switches/second graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_ctxtsw.$2'"' >> $tmpFile;
  echo 'set title "Context switches"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of context switches per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.ctxsw.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Disk total transfers
  echo "Generating Servlets Server's Disk total transfers graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_disk_tps.$2'"' >> $tmpFile;
  echo 'set title "Disk transfers"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of disk transfers per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.disk.tps.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot disk read/write requests
  echo "Generating Servlets Server's disk read/write requests graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_disk_rw_req.$2'"' >> $tmpFile;
  echo 'set title "Read/Write disk requests"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of requests per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.disk.rtps.dat'"' title '"'Servlets Server read'"' with lines, '"'$1servlets_server.disk.wtps.dat'"' title '"'Servlets Server write'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot disk blocks read/write requests
  echo "Generating Servlets Server's disk blocks read/write requests graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_disk_rw_req.$2'"' >> $tmpFile;
  echo 'set title " Disk blocks read/write requests"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of blocks per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.disk.brdps.dat'"' title '"'Servlets Server read'"' with lines, '"'$1servlets_server.disk.bwrps.dat'"' title '"'Servlets Server write'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Memory usage
  echo "Generating Servlets Server's Memory usage graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_mem_usage.$2'"' >> $tmpFile;
  echo 'set title "Memory usage"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Amount of memory in KB"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.mem.kbmemused.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Memory & cache usage
  echo "Generating Servlets Server's Memory & cache usage graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_mem_cache.$2'"' >> $tmpFile;
  echo 'set title "Memory & cache usage"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Amount of memory in KB"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.mem.kbmemused.dat'"' title '"'Servlets Server memory'"' with lines, '"'$1servlets_server.mem.kbcached.dat'"' title '"'Servlets Server cache'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot network received/transmitted packets
  echo "Generating Servlets Server's network received/transmitted packets graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_net_rt_pack.$2'"' >> $tmpFile;
  echo 'set title "Network received/transmitted packets"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of packets per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.net.rxpck.dat'"' title '"'Servlets Server received'"' with lines, '"'$1servlets_server.net.txpck.dat'"' title '"'Servlets Server transmitted'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot network received/transmitted bytes
  echo "Generating Servlets Server's network received/transmitted bytes graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_net_rt_byt.$2'"' >> $tmpFile;
  echo 'set title "Network received/transmitted bytes"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Nb of bytes per second"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.net.rxbyt.dat'"' title '"'Servlets Server received'"' with lines, '"'$1servlets_server.net.txbyt.dat'"' title '"'Servlets Server transmitted'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile

  # Plot Sockets usage
  echo "Generating Servlets Server's Sockets usage graph";
  echo "set terminal "$2 > $tmpFile;
  echo set output '"'$1servlets_server_socks.$2'"' >> $tmpFile;
  echo 'set title "Sockets"' >> $tmpFile;
  echo 'set xlabel "Time in seconds"' >> $tmpFile;
  echo 'set ylabel "Number of sockets"' >> $tmpFile;
  echo -n plot '"'$1servlets_server.sock.totsck.dat'"' title '"'Servlets Server'"' with lines >> $tmpFile;
  /usr/bin/gnuplot $tmpFile
fi
