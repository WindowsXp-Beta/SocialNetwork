...
public class Search extends RubbosHttpServlet
{
      ...
      throws IOException, ServletException
  {
    ...
    //search = "user123"; // add by qingyang and try to verify the response time of search requests. 

    if (testnbOfStories == null)
      nbOfStories = 25;
    nbOfStories = 0;  //add by qingyang and try to verify the response time of search requests
      ...
      if (type == 0)
      {
        try
        {
          stmt = conn.prepareStatement("SELECT stories.id, stories.title, "
              ...
              + " stories, users WHERE title = '" + search + "%' "
              + "AND stories.writer = users.id" + /* OR body = '$search%%' */
          ...
          if (!rs.first())
          {
            ...
            stmt = conn
                .prepareStatement("SELECT old_stories.id, old_stories.title,"
                    ...
                    + " old_stories, users WHERE title = '" + search + "%' "
