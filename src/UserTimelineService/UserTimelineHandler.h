...
void UserTimelineHandler::WriteUserTimeline(
  ...
  LOG(info) << "UserTimeline2WriteUserTimeline-MongoDB conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  conn_starttime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
  try {
    if (_redis_client_pool) {
      _redis_client_pool->zadd(std::to_string(user_id), std::to_string(post_id),
                              timestamp, UpdateType::NOT_EXIST);
      auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      LOG(info) << "UserTimeline2WriteUserTimeline-Redis conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
    } else {
  ...
  LOG(info) << "UserTimeline2WriteUserTimeline starttime=" << starttime << " endtime=" << endtime;
...
void UserTimelineHandler::ReadUserTimeline(
  ...
  try {
    if (_redis_client_pool) {
      ...
      auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      LOG(info) << "UserTimeline2ReadUserTimeline-Redis conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;                            
    } else {
  ...
  if (mongo_start < stop) {
    ...
    auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
    LOG(info) << "UserTimeline2ReadUserTimeline-MongoDB conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
      ...
      std::async(std::launch::async, [&]() {
        ...
        try {
          ...
          post_client->ReadPosts(_return_posts, req_id, post_ids,
                                 writer_text_map);
	        auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
	        LOG(info) << "UserTimeline2ReadUserTimeline-PostStorage conn_starttime=" << conn_starttime << " conn_endtime=" << conn_endtime;
  ...
  if (redis_update_map.size() > 0) {
    ...
    try {
      if (_redis_client_pool) {
        ...
        auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
	      LOG(info) << "UserTimeline2ReadUserTimeline-UpdateRedis conn_starttime=" << conn_starttime << " conn_endtime=" << conn_endtime;
      } else {
  ...
  auto endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
  LOG(info) << "UserTimeline2ReadUserTimeline starttime=" << starttime << " endtime=" << endtime;
