...
void TextHandler::ComposeText(
  ...
  auto shortened_urls_future = std::async(std::launch::async, [&]() {
    ...
    try {
      ...
      LOG(info) << " TextService2ComposeText-urlShortenService conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  auto user_mention_future = std::async(std::launch::async, [&]() {
    ...
    try {
      ...
      auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      LOG(info) << " TextService2ComposeText-userMentionService conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  LOG(info) << "TextService2ComposeText starttime=" << starttime << " endtime=" << endtime;
