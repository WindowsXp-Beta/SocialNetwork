...
void UserMentionHandler::ComposeUserMentions(
  ...
  if (!usernames.empty()) {
    ...
    LOG(info) << "UserMention2ComposeUserMentions-Memcached conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
    ...
    if (!usernames_not_cached.empty()) {
      ...
      auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      LOG(info) << "UserMention2ComposeUserMentions-MongoDB conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  LOG(info) << "UserMention2ComposeUserMentions starttime=" << starttime << " endtime=" << endtime;
