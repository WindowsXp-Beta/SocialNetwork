...
class ClientPool {
 public:
  ...
  void Remove(TClient *);
  void Report(TClient *, int* cur_pool, int* max_pool);
...
template<class TClient>
void ClientPool<TClient>::Report(TClient *, int* cur_pool, int* max_pool) {
     *cur_pool = _curr_pool_size;
     *max_pool = _max_pool_size;
}
} // namespace social_network
