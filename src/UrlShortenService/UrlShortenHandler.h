...
void UrlShortenHandler::ComposeUrls(
  ...
  if (!urls.empty()) {
    ...
    mongo_future = std::async(
        std::launch::async, [&](){
          ...
          LOG(info) << "UrlShorten2ComposeUrls-insertMongoDB conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  if (!urls.empty()) {
    try {
      ...
      mongo_future.get();
      auto conn_endtime = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      LOG(info) << "UrlShorten2ComposeUrls-uploadMongoDB conn_starttime=" << conn_starttime << "conn_endtime=" << conn_endtime;
  ...
  LOG(info) << "UrlShorten2ComposeUrls starttime=" << starttime << " endtime=" << endtime;
