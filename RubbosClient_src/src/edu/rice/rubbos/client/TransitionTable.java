...
/**
 * This class provides support for transitions between RUBBoS web site pages. A
 * matrix contains probabilities of transition from one state to another. A
 * ReadExcelTextFile() method generates a matrix from an Excel file saved as
 * text with tab separator. The text file must have the following format :
 * 
 * <pre>
 ...
 * <code>probability_x_y</code> determines the probability to go from state x to
 * state y.
 * 
 * There are 2 extra lines compared to colums. These lines are:
 * <code>Back probability:</code> probability to go back to last page (like the
 * back button of the browser) <code>End of session:</code> probability that the
 * user ends the session (leave the web site).
 * </pre>
 *
...
public class TransitionTable {
	...
	public int nextState() {
		...
		if (currentState == nbRows - 2) {
			if (previousStates.empty())
				System.out.println("Error detected: Trying to go back but no previous state is available (currentState:"
						+ currentState + ", beforeStep:" + beforeStep);
			else { // Back adds both stats of back and new state but only sleep
					// "back waiting time"
					// and return the new state (after back).
					// Yasu:
					// stats.incrementCount(currentState); // Add back state
					// stat
				try {
					long thinktime = 0;
					if (useTPCWThinkTime) {						
						// Thread.currentThread().sleep(TPCWthinkTime());
						thinktime = (long) (TPCWthinkTime() * ClientEmulator.getSlowDownFactor());
						Thread.sleep(thinktime);
						//System.out.println("Normal think time:"+thinktime);
					} else{
						thinktime = (long) ((float) transitionsTime[currentState] * ClientEmulator.getSlowDownFactor());
						Thread.sleep(thinktime);
						//System.out.println("no think time:"+thinktime);
					}
				...
				// "+Thread.currentThread().getName()+": Going back from
				// "+stateNames[beforeStep]+" to
			...
			if (!isEndOfSession()) { // If there is no probability to go back
										// from this state, just empty the stack
				if (transitions[currentState][nbRows - 2] == 0)
					previousStates.removeAllElements();
				else // else add the previous state to the history just in case
						// we go back !
					previousStates.push(new Integer(beforeStep));
				// System.out.println("Thread
				// "+Thread.currentThread().getName()+":
				// "+stateNames[beforeStep]+" ->
				// "+stateNames[currentState]+"<br>\n");
			}
		}
		// Yasu:
		...
		try {
			if (useTPCWThinkTime)
				// Thread.currentThread().sleep(TPCWthinkTime());
				Thread.currentThread().sleep((long) (TPCWthinkTime() * ClientEmulator.getSlowDownFactor()));
			else
				Thread.currentThread()
						.sleep((long) ((float) transitionsTime[currentState] * ClientEmulator.getSlowDownFactor()));
		} catch (java.lang.InterruptedException ie) {
			System.err.println("Thread " + Thread.currentThread().getName() + " has been interrupted.");
		}
		return currentState;
	...
	/**
	 * Read the matrix transition from a file conforming to the format described
	 * in the class description.
	 *
	 * @param filename
	 *            name of the file to read the matrix from
	...
	public boolean ReadExcelTextFile(String filename) {
		...
		try {
			...
			for (i = 0; i < nbRows; i++) {
				...
				transitionsTime[i] = t.intValue(); // TBF: t.intValue();
			}
			reader.close();
			...
			System.err.println("File format error in file " + filename + " when reading line " + i + ", column " + j
					+ ". (" + nsu.getMessage() + ")");
			return false;
		} catch (NumberFormatException ne) {
			System.err.println("Number format error in file " + filename + " when reading line " + i + ", column " + j
					+ ". (" + ne.getMessage() + ")");
			return false;
		}
	 ...
	 *
	 * @param title
	 *            transition table name to be displayed in title
	 */
	...
	private long TPCWthinkTime() {
		...
		return ((long) ((((double) (-1) * PCWThinkTime) * Math.log(r)) + 0.5));
	}

	public void setPCWThinkTime(int thinkingTime) {
		this.PCWThinkTime = thinkingTime;
	}

	public int getPCWThinkTime() {
		return this.PCWThinkTime;
	}
