...
public class RUBBoSProperties
{
  ...
  private String servletsSiteName;
  ...
  private Vector  databaseServers;
  private int nbOfDatabaseServers;
  ...
  public PrintStream buddy = null;
  

  
  private int TCPthinkingTime = 0; //add by Qingyang
  /**
  ...
  public URLGenerator checkPropertiesFileAndGetURLGenerator()
  {
    try
    {
      ...
      System.out.print("Servlets server name : ");
      servletsSiteName  = getProperty("servlets_server");
      System.out.println(servletsSiteName+"<br>");
      ...
      /* add by Qingyang*/
      servletsSingleServletPath  = getProperty("servlets_singleServlet_path");
      System.out.print("servlets_singleServlet_path : ");
      System.out.println(servletsSingleServletPath+"<br>");
      foo = new Integer(getProperty("workload_tpcw_think_time"));
      TCPthinkingTime = foo.intValue();
      System.out.print("TCPthinkingTime : ");
      System.out.println(TCPthinkingTime+"<br>");
      ...
      StringTokenizer servlets_nodes = new StringTokenizer(getProperty("servlets_servers"),",");
      servletsServers = new Vector();
      while (servlets_nodes.hasMoreTokens())
    	  servletsServers.add(servlets_nodes.nextToken().trim());
            
      nbOfServletServers = servletsServers.size();
      System.out.println(nbOfServletServers+"<br>");
      
      
      // # Workload
      System.out.println("\n<h3><br>### Workload ###</h3>");
      benchmarkNode  = getProperty("workload_benchmark_node");
      System.out.println("\n Benchmark Node          : " + benchmarkNode + "<br>");      
      ...
      //add by Hui
	  if(configuration.containsKey("workload_dyn")){
		  loads = new StringTokenizer(getProperty("workload_dyn"),",");
	      workloads = new int[loads.countTokens()];
	      int i = 0;
	      StringBuilder loadstring = new StringBuilder();
	      while (loads.hasMoreTokens()){
	    	  workloads[i++]=Integer.valueOf(loads.nextToken().trim());
	    	  loadstring.append(String.valueOf(workloads[i-1]));
	      }
	      System.out.println("workloads array     :"+loadstring.toString()+" tokens:"+loads.countTokens()+"<br>");
	  }
      ...
      //
      //
      // Modified this line to allow replication
      // START OF NEW CODE
      dbServerName = getProperty("database_master_server");
      ...
      StringTokenizer database_nodes = new StringTokenizer(getProperty("database_servers"),",");
      databaseServers = new Vector();
      while (database_nodes.hasMoreTokens())
    	  databaseServers.add(database_nodes.nextToken().trim());


      StringTokenizer slaves = new StringTokenizer(getProperty("database_slave_servers"),",");

      databaseSlaves = new Vector();

      while (slaves.hasMoreTokens())
        databaseSlaves.add(slaves.nextToken().trim());

      nbOfSlaves = databaseSlaves.size();
...
//         urlGen = new URLGeneratorEJB(webSiteName, webSitePort, EJBHTMLPath, EJBScriptPath);
  ...
  public Vector getServletsServers()
  {
    return servletsServers;
  }
  
  public Vector getDBServers()
  {
  ...
  public String getbenchmarkNode() {
	// TODO Auto-generated method stub
	return benchmarkNode;
  }

  public String getServletsSingleServletPath() {
		// TODO Auto-generated method stub
		return servletsSingleServletPath;
	  }


public int getThinkingTime() {
	// TODO Auto-generated method stub
	return TCPthinkingTime;
}
