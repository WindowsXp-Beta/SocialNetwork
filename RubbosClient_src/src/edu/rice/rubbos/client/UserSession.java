...
public class UserSession extends Thread
{
	...
	public UserSession(String threadId, URLGenerator URLGen, RUBBoSProperties RUBBoS, Stats statistics, boolean isAuthor, TransitionTable theTransition)
	{
		...
		*/
		//add by hchen
        if(rubbos.useTPCWThinkTime())
            transition.setPCWThinkTime(rubbos.getThinkingTime());
	...
	private String callHTTPServer(URL url)
	{
		...
		long                stime, etime, tmp_time;
                long                time1, time2, time3, time5, time6;
		...
		while (retry < 5)
		{
			...
			try
			{
				...
				System.err.println(logMessage);		//Used for ip mapping
			...
			logMessage += "ConnectionOpen: " + tmp_time;
                        time1 = System.currentTimeMillis();
		...
		etime = System.currentTimeMillis();	// Move endtime to here for sockshop
		tmp_time = System.currentTimeMillis();
		logMessage += "ConnectionClose: " + tmp_time;
		...
		//if (rt > 3000) {
			...
			//System.err.println(logMessage);
		//}
                //System.out.println("stime " + stime + " Open Connection End " + time1 + " Getting Data " + time5 + " " + time6 + " " + " Get Data End " + time2 + " Close Connection End" + time3 + " etime " + etime);
	...
	public synchronized void setEndOfSimulation()
	{
	...
	public void run()
	{
		...
		boolean init_firsttime_sleep = true;
		...
		while (!isEndOfSimulation())
		{
			...
			//add by Qingyang, the purpose is to avoid every request starting at the same time.
			if(init_firsttime_sleep){
				try {
				...
				init_firsttime_sleep = false;
			}


			next = transition.getCurrentState();
			while (!isEndOfSimulation() && !transition.isEndOfSession() && (nbOfTransitions > 0))
			{
				// Compute next step and call HTTP server (also measure time spend in server call)
				//lastURL = computeURLFromState(next);
				//lastURL = singleTypeURLGenerator();  // add by Qingyang
				//lastURL = doubleTypeURLGenerator();   //add by Jianshu
				String tmpUrl = servletP();
                                lastURL = sockshopURLGenerator(tmpUrl);
                                if (lastURL == null)
				{
					// Yasu:
					...
					transition.resetToInitialState();
					//lastURL = urlGen.homePage();  // add by Qingyang
					//lastURL = singleTypeURLGenerator();
					//lastURL = doubleTypeURLGenerator();
					lastURL = sockshopURLGenerator(tmpUrl);
				}
				...
				// Yasu: Only in success case, we can increase the amount of time
					...
					stats.addUrl(next, tmpUrl);	//Xuhang
	...
	private URL singleTypeURLGenerator() {

		//String servletPath = rubbos.getServletsSingleServletPath();
		//if (servletPath == "" || servletPath == null ){
			//servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.StoriesOfTheDay";
		String servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.StoriesOfTheDay";
                //String servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.ViewStory?storyId=3253";
		//}
		URL url = null;
       ...
       private URL doubleTypeURLGenerator() {


               //String light_servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.StoriesOfTheDay";
               //String light_servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.ViewComment?commentId=157987&filter=0&display=1&storyId=4445&comment_table=comments";
               //String heavy_servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.ViewStory?storyId=3253";
               String heavy_servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.JLWrite";
               String light_servletPath = "/rubbos/servlet/edu.rice.rubbos.servlets.JLSearch";
               ...
               int port  = 9000;
                ...
                try
                {
                        ...
                        if (i>=0){
                           url = new URL("http", hostname, port, heavy_servletPath);
                        }else{
                           url = new URL("http", hostname, port, light_servletPath);
                        }
                }
                catch (java.net.MalformedURLException e)
                {
	...
	private String servletP() {
		String path = "/";
		Random rand = new Random();

		int user_id;
		// if (dist < 10) {	// readUser
		user_id = rand.nextInt(800);
		int start = 1;

		int stop = start + 1;

		path = "/wrk2-api/home-timeline/read?user_id=" + Integer.toString(user_id) + "&start=" + Integer.toString(start) + "&stop=" + Integer.toString(stop);

		return path;
	}

       private URL sockshopURLGenerator(String servletPath){
		//servletPath
