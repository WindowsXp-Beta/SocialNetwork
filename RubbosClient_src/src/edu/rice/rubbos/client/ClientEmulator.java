...
public class ClientEmulator
{
	...
	private static float    slowdownFactor = 1;
	...
	public ClientEmulator()
	{
		try{
			...
			System.out.println("buddy is created");
			e.printStackTrace();
		}
	...
	public static void main(String[] args)
	{
		...
		// Start pre-runtime session, this is to mitigate the impact of transition of think time from up-ramp to runtime. 
		System.out.println("<br><A NAME=\"up\"></A>");
		System.out.println("<h3>ClientEmulator: Switching to ** pre-runtime session **</h3><br><p>");
		client.setSlowDownFactor(1);
		upRampDate = new GregorianCalendar();
		try
		{
			// 60 seconds should be enough for all the threads to have the run-time think time.  
		...
		// add by hchen,write results in a txt file
		if(isMainClient){
			String path=reportDir+"/result0.jtl";
			runSessionStats.getresult(path,TimeManagement.diffTimeInMs(runSessionDate,downRampDate));
		}
		else
			runSessionStats.getresult(args[2],TimeManagement.diffTimeInMs(runSessionDate,downRampDate));
